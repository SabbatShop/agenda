...
export default function HabitTracker({ habits, onToggle, onAdd, onRemove, onEdit }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (newHabit.trim()) {
      onAdd(newHabit, intervalTime ? parseInt(intervalTime) : null, newEmoji);
      setNewHabit(''); setIntervalTime(''); setNewEmoji('✨');
      setIsAdding(false); setShowEmojiPicker(false);
    }
  };
  ...
  const handleSaveEdit = (e, id) => {
    e.preventDefault();
    if (editTitle.trim()) {
      onEdit(id, editTitle, editInterval ? parseInt(editInterval) : null, editEmoji);
      setEditingId(null);
      setShowEditEmojiPicker(false);
    }
  };

  const handleDeleteWithUndo = (habit) => {
    onRemove(habit.id);
    toast((t) => (
      <div className="flex items-center gap-3">
        <span>"{habit.title}" removido</span>
        <button
          onClick={() => {
            onAdd(habit.title, habit.intervalMinutes, habit.emoji);
            toast.dismiss(t.id);
          }}
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-slate-800 dark:text-zinc-100">Rotina Diária</h2>
        <button
      ...
      <AnimatePresence>
        {isAdding && (
          <motion.form
            ...
            className="mb-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-3 rounded-2xl flex flex-col gap-3 overflow-hidden"
          >
            {/* Seletor de Emoji */}
            <div>
              <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)}
                className="text-2xl p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors" title="Escolher emoji">
                {newEmoji}
              </button>
              {showEmojiPicker && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {EMOJI_OPTIONS.map(e => (
                    <button key={e} type="button" onClick={() => { setNewEmoji(e); setShowEmojiPicker(false); }}
                      className="text-xl p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors active:scale-90">
                      {e}
                    </button>
                  ))}
                </div>
            ...
            <input
              type="text" autoFocus value={newHabit}
            ...
            <div className="flex gap-2">
              <input
                type="number" value={intervalTime}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {habits.map((habit) => (
          <div key={habit.id} className="relative group flex flex-col">
            {editingId === habit.id ? (
              <form onSubmit={(e) => handleSaveEdit(e, habit.id)}
                className="bg-white dark:bg-zinc-900 border border-green-300 dark:border-green-700 p-3 rounded-2xl flex flex-col gap-2 shadow-sm z-10">
                {/* Emoji no edit */}
                <div>
                  <button type="button" onClick={() => setShowEditEmojiPicker(!showEditEmojiPicker)}
                    className="text-2xl p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
                    {editEmoji}
                  </button>
                  {showEditEmojiPicker && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {EMOJI_OPTIONS.map(e => (
                        <button key={e} type="button" onClick={() => { setEditEmoji(e); setShowEditEmojiPicker(false); }}
                          className="text-xl p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 active:scale-90">
                          {e}
                        </button>
                      ))}
                    </div>
                ...
                <input type="text" autoFocus value={editTitle}
                  onChange={(e) => setEditTitle(e.target.value)} placeholder="Nome do hábito"
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <input type="number" value={editInterval}
                    onChange={(e) => setEditInterval(e.target.value)} placeholder="Minutos"
                ...
                <div className="flex gap-2 mt-1">
                  <button type="button" onClick={() => setEditingId(null)}
                    className="flex-1 py-1.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-bold">Cancelar</button>
                  <button type="submit"
                    className="flex-1 py-1.5 bg-green-500 text-white rounded-lg text-xs font-bold">Salvar</button>
                </div>
              </form>
            ) : (
              <div className="flex-1 flex relative">
                ...
                >
                  {/* Emoji ou Check */}
                  <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-lg transition-colors ${
                    habit.completed ? 'bg-green-500' : 'bg-slate-100 dark:bg-zinc-800'
                  }`}>
                    {habit.completed ? <Check size={16} strokeWidth={3} className="text-white" /> : (habit.emoji || '✨')}
                  </div>

                  <div className="flex flex-col items-start text-left min-w-0">
                    <span className={`text-sm font-semibold truncate w-full transition-all ${habit.completed ? 'line-through decoration-2 decoration-green-500/40 text-green-700 dark:text-green-500' : 'text-slate-700 dark:text-slate-300'}`}>
                      {habit.title}
                    </span>
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-1">
                  ...
                  <button onClick={() => handleDeleteWithUndo(habit)}
